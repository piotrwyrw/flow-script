import { Token, TokenMappingStrategies, TokenTypes } from "./token";
export class Tokenizer {
    ...
    createTokenFrom(token) {
        return new Token(token.type, token.value, token.line, token.col);
    }
    // Find a token type whose `mapping` directly matches the value
    findFirstDirectMappingTokenType(mappingStrategy, value) {
        for (let typeKey in TokenTypes) {
            if (!TokenTypes.hasOwnProperty(typeKey)) {
                continue;
            }
            const tokTypeKey = typeKey;
    ...
    findBestMatchTokenType(mappingStrategy, value) {
        let bestMatch;
        for (let typeKey in TokenTypes) {
            if (!TokenTypes.hasOwnProperty(typeKey)) {
                continue;
    ...
    processGenericSymbolToken(token) {
    ...
    tokenize() {
        ...
        const acquireChar = () => {
            ...
            if (!current)
                throw new Error(`Tokenizer encountered undefined character at ${this.line}:${this.column}`);
        };
        while (this.input.length !== 0) {
            ...
            acquireChar();
            if (!current)
                throw new Error("Tokenizer encountered unexpected undefined character. This should not happen.");
